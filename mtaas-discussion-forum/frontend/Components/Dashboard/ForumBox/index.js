...
class ForumBox extends Component {
  ...
  handleIndividualForum(forumid) {
    // remove any error messages
    console.log("Inside handleIndividualForum");
    console.log(forumid);
    // this.setState({ errorMsg: null });
    ...
    return (
      <div className={styles.container}>
        ...
        <div className={styles.forumsContainer}>
          {deletingForum && <div className={styles.loadingMsg}>Removing Project, please wait...</div>}

          {forums.map((forum) => <div onClick={() => { this.handleIndividualForum(forum.slug.value) }} key={forum.id} className={styles.forum}>
            <div className={styles.forumTitle}>{forum.name}</div>
            <div className={styles.forumSlug}>({forum.slug})</div>
            <div className={styles.removeButton}>
              {/* <Button name={forum.id} onClick={() => { this.handleIndividualForum(this.name) }}>Select</Button> */}
            </div>
          </div>)}
