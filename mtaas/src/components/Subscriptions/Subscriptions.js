...
class Subscriptions extends React.Component {
  constructor(props) {
    ...
    axios.post(hostedAddress+':3001/getSubscribedProjectsForTester',data, {params:{},headers:{'Authorization':token, 'Accept':'application/json','Content-Type':'application/json'}})
      .then((response) => {
  ...
  apply = arr => {
    alert(
      "Your profile will be shared with the Manager. Are you sure you want to apply?"
    );
    let token = localStorage.getItem("bearer-token");
    let data = {
    ...
    axios.defaults.withCredentials = true; //very imp, sets credentials so that backend can load cookies
    axios
      .post(hostedAddress + ":3001/applyForProject", data, {
        params: {},
        headers: {
          Authorization: token,
      ...
      <div className="mainDiv">
        ...
        <div className="homeDiv">
          <div style={{ marginTop: "1%" }}>
            <Jumbotron>
              <Container>
                <h4 className="display-4">Subscriptions</h4>
              </Container>
            </Jumbotron>
          </div>
        </div>
        <div className="listDiv">
          <div>
            <div>
              <div class="card-arrange">{allProjCards}</div>
            </div>
          </div>
        </div>
      </div>
